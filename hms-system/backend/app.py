...
DB_PATH = os.getenv('DB_PATH', 'hospital_db.sqlite')

# Enable SQLite to use dictionary-like rows
...
class CustomJSONProvider(DefaultJSONProvider):
    def default(self, obj):
        # Dict-like rows from SQLite dict_factory
...
# ============================================
# Database Connection Layer (Unified)
# ============================================
...
def get_db_connection():
    ...
    try:
        ...
        print(f"Database connection error (Postgres={IS_POSTGRES}): {e}")
        raise
...
class CompatibleCursor:
    """Wrapper for database cursor to handle SQLite/Postgres compatibility automatically"""
    def __init__(self, cursor, conn):
        self.cursor = cursor
        self.conn = conn
        self.IS_POSTGRES = DATABASE_URL and DATABASE_URL.startswith('postgresql')
    ...
    def execute(self, query, params=None):
        ...
        if self.IS_POSTGRES:
            # Advanced Interval and modifier conversions
            import re
            # Handle date('now', 'localtime', '-' || ? || ' days')
            query = re.sub(r"date\(['\"]now['\"],\s*['\"]localtime['\"],\s*['\"]-['\"]\s*\|\|\s*([^|]+)\s*\|\|\s*['\"] days['\"]\)", r"CURRENT_DATE - ((\1) || ' days')::INTERVAL", query)
            # Handle date('now', '-' || ? || ' days')
            query = re.sub(r"date\(['\"]now['\"],\s*['\"]-['\"]\s*\|\|\s*([^|]+)\s*\|\|\s*['\"] days['\"]\)", r"CURRENT_DATE - ((\1) || ' days')::INTERVAL", query)
            
            # Handle date('now', '-30 days')
            query = re.sub(r"date\(['\"]now['\"],\s*['\"]-(\d+)\s+days['\"]\)", r"CURRENT_DATE - INTERVAL '\1 days'", query)
            query = re.sub(r"date\(['\"]now['\"],\s*['\"]-(\d+)\s+months['\"]\)", r"CURRENT_DATE - INTERVAL '\1 months'", query)
            
            # Handle start of month/year
            query = re.sub(r"date\(['\"]now['\"],\s*['\"]star[t] of month['\"]\)", r"date_trunc('month', CURRENT_DATE)", query, flags=re.IGNORECASE)
            query = re.sub(r"date\(['\"]now['\"],\s*['\"]star[t] of year['\"]\)", r"date_trunc('year', CURRENT_DATE)", query, flags=re.IGNORECASE)
            
            # Handle DATE(col, 'localtime') -> col::date
            query = re.sub(r"DATE\(([^,)]+),\s*['\"]localtime['\"]\)", r"(\1)::DATE", query, flags=re.IGNORECASE)
            
            # Handle simple date('now', 'localtime')
            query = query.replace("date('now', 'localtime')", "CURRENT_DATE")
            query = query.replace('date("now", "localtime")', "CURRENT_DATE")
            
            # General fallback for date('now', ...) with 2+ args if not matched above
            query = re.sub(r"date\(['\"]now['\"],\s*[^)]+\)", r"CURRENT_DATE", query)
            
            # Handle DATE(column) - some PG versions prefer column::date
            # but DATE() usually works. The error was about date(unknown, unknown) though.
            
            # Date and Time conversions (exact matches first)
            ...
            # Automate RETURNING id for lastrowid support
            if is_insert and "RETURNING" not in query.upper():
                # Primitive check for 'id' column existence - most tables in this schema have it
                query = query.rstrip('; ') + " RETURNING id"
        else:
            # SQLite compatibility for Postgres-style keywords
        ...
        try:
            ...
            # Capture lastrowid for Postgres if we added RETURNING
            if self.IS_POSTGRES and is_insert:
                try:
                    ...
                    # If fetchone fails (e.g. no RETURNING happened), ignore
                    pass
            
            return result
        except Exception as e:
            # If the RETURNING id failed (e.g. no 'id' column), retry without it
            if self.IS_POSTGRES and is_insert and "column \"id\" does not exist" in str(e):
                try:
                    retry_query = original_query.replace('?', '%s')
                    if params:
                        return self.cursor.execute(retry_query, params)
                    else:
                        return self.cursor.execute(retry_query)
                except Exception:
                    pass
            print(f"CompatibleCursor Error: {e}\nQuery: {query}\nParams: {params}")
            raise
...
def get_dict_cursor(conn):
    """Return a compatible cursor that returns rows as dictionaries"""
    if IS_POSTGRES:
        raw_cursor = conn.cursor(cursor_factory=RealDictCursor)
    else:
        raw_cursor = conn.cursor()
    return CompatibleCursor(raw_cursor, conn)

def execute_query(conn, query, params=None):
    """Execute a query and return the compatible cursor"""
...
def init_db():
    """Verify database connection and initialize schema if needed"""
    if IS_POSTGRES:
        ...
        try:
            ...
            print(f'PostgreSQL init error: {e}. Check DATABASE_URL.')
            sys.exit(1)
    else:
        print('Using SQLite (hospital_db.sqlite)')
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            ...
            print(f'SQLite init error: {e}. Check DB_PATH.')
            sys.exit(1)

# Initialize DB on start and ensure any lightweight support tables exist.
# The full production schema is deployed separately via SQL migration files.

def ensure_helper_tables():
    """Create a few small tables that may not be present in a fresh database."""
    try:
        conn = get_db_connection()
        cursor = get_dict_cursor(conn)
        cursor.execute("""
        ...
        print("  helper tables ensured (registration_otps)")
    except Exception as e:
        print(f"Warning: helper table creation failed: {e}")

try:
    init_db()
    ensure_helper_tables()
except Exception as e:
    print(f"Warning: init_db or helper table setup failed: {e}. Continuing anyway.")
...
def patient_send_otp():
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM patients 
            WHERE (patient_id = ? OR mobile_number = ?) AND is_active = TRUE
        ...
        cursor.execute(
            "UPDATE patients SET current_otp = ?, otp_expiry = ? WHERE patient_id = ?",
...
def staff_login():
    ...
    try:
        ...
        # Check password (wrap in try/except to avoid internal errors leaking)
        try:
...
def register_patient():
    ...
    try:
        ...
        cursor = execute_query(conn, "SELECT id FROM patients WHERE mobile_number = ?", (mobile_number,))
        if cursor.fetchone():
            return jsonify({'error': 'Mobile number already registered.'}), 409
            
        # Verify OTP if not bypassed
        if not bypass_otp:
            cursor = execute_query(conn, "SELECT otp, expiry FROM registration_otps WHERE mobile_number = ?", (mobile_number,))
        ...
        # Generate patient_id
        cursor = execute_query(conn, "SELECT patient_id FROM patients ORDER BY id DESC LIMIT 1")
        ...
        if not bypass_otp:
            execute_query(conn, "DELETE FROM registration_otps WHERE mobile_number = ?", (mobile_number,))
        
        # Set phone verified to 1
        execute_query(conn, "UPDATE patients SET phone_verified = ? WHERE id = ?", (True, pk_id))
...
def create_appointment():
    ...
    try:
        ...
        cursor = execute_query(conn, "SELECT registered_by FROM patients WHERE patient_id = ?", (patient_id,))
        ...
        if status == 'Confirmed':
            ...
            cursor = execute_query(conn, "UPDATE appointments SET token_number = ? WHERE appointment_id = ?", (token_number, appointment_id))
...
def get_doctor_dashboard():
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE queue_date = CURRENT_DATE AND (status = 'Waiting' OR status = 'In_Progress')
        """)
        ...
        cursor.execute("""
            SELECT COUNT(*) as count FROM queue_management
            WHERE queue_date = CURRENT_DATE AND status = 'Completed'
        """)
        ...
        cursor.execute("""
            ...
            WHERE status IN ('Results_Entered', 'Verified') AND DATE(updated_at) = CURRENT_DATE
        """)
        ...
        cursor.execute("""
            ...
            JOIN patients p ON q.patient_id = p.patient_id
            JOIN appointments a ON q.appointment_id = a.appointment_id
            WHERE q.doctor_id = %s AND q.queue_date = CURRENT_DATE
...
def get_front_office_dashboard():
    """Get front office dashboard data with camelCase keys for React frontend"""
    conn = get_db_connection()
    cursor = get_dict_cursor(conn)
    
    try:
        # Today's stats
        cursor = execute_query(conn, "SELECT COUNT(*) as count FROM patients WHERE DATE(registration_date) = CURRENT_DATE")
        today_registrations = cursor.fetchone()['count']
        
        cursor = execute_query(conn, "SELECT COUNT(*) as count FROM appointments WHERE appointment_date = CURRENT_DATE")
        today_appointments = cursor.fetchone()['count']
        
        cursor = execute_query(conn, "SELECT COUNT(*) as count FROM pending_payments WHERE status = 'Pending'")
        pending_payments_count = cursor.fetchone()['count']
        
        cursor = execute_query(conn, "SELECT COALESCE(SUM(amount), 0) as total FROM collections WHERE DATE(collected_at) = CURRENT_DATE")
        today_revenue = cursor.fetchone()['total']
        
        # General stats
        cursor = execute_query(conn, "SELECT COUNT(*) as count FROM patients")
        total_patients = cursor.fetchone()['count']
        
        cursor = execute_query(conn, "SELECT COUNT(*) as count FROM queue_management WHERE queue_date = CURRENT_DATE AND status = 'Waiting'")
        waiting_queue = cursor.fetchone()['count']
        
        # Weekly stats
        cursor = execute_query(conn, "SELECT COUNT(*) as count FROM patients WHERE DATE(registration_date) >= date('now', '-7 days')")
        weekly_registrations = cursor.fetchone()['count']
        
        cursor = execute_query(conn, "SELECT COUNT(*) as count FROM appointments WHERE appointment_date >= date('now', '-7 days')")
        weekly_appointments = cursor.fetchone()['count']
        
        cursor = execute_query(conn, "SELECT COALESCE(SUM(amount), 0) as total FROM collections WHERE DATE(collected_at) >= date('now', '-7 days')")
        weekly_revenue = cursor.fetchone()['total']
        
        # Monthly stats
        cursor = execute_query(conn, "SELECT COALESCE(SUM(amount), 0) as total FROM collections WHERE DATE(collected_at) >= date('now', '-30 days')")
        ...
        print(f"Front Office Dashboard error: {e}")
        return jsonify({'error': str(e)}), 500
    finally:
        cursor.close()
        close_db_connection(conn)
    ...
    try:
        cursor = execute_query(conn, """
            ...
            SELECT 
                ...
                p.first_name || ' ' || p.last_name as patientName,
                'Appointment with Dr. ' || s.last_name as description,
            ...
            SELECT 
                ...
                p.first_name || ' ' || p.last_name as patientName,
                'Fee collected: ₹' || CAST(c.amount as TEXT) as description,
...
def search_front_office_payments():
    ...
    cursor = get_dict_cursor(conn)
    try:
        # Simple search across name and ID
        cursor = execute_query(conn, """
            SELECT 
                pp.id,
                pp.patient_id,
            ...
            JOIN patients p ON pp.patient_id = p.patient_id
            WHERE pp.status = 'Pending' 
            AND (p.first_name ILIKE ? OR p.last_name ILIKE ? OR pp.patient_id ILIKE ?)
            ORDER BY pp.created_at DESC
...
@jwt_required()
def collect_front_office_payment():
    data = request.json
    payment_id = data.get('payment_id')
    ...
    try:
        ...
        # Insert into collections
        cursor.execute("""
            INSERT INTO collections (payment_id, patient_id, amount, method, transaction_id, collected_by)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (payment_id, patient_id, amount, method, transaction_id, staff_id))
        ...
        return jsonify({
            'message': 'Payment collected successfully',
            'receipt_number': f'REC-{collection_id:06d}'
        }), 200
    except Exception as e:
        conn.rollback()
        return jsonify({'error': str(e)}), 500
...
def get_doctor_stats():
    ...
    try:
        ...
        cursor.execute("""
            SELECT AVG((strftime('%s', consultation_end_time) - strftime('%s', consultation_start_time)) / 60) as avg_time
            FROM appointments
            WHERE doctor_id = ? AND status = 'Completed' 
            AND consultation_end_time IS NOT NULL AND consultation_start_time IS NOT NULL
        ...
        cursor.execute("SELECT rating FROM doctors WHERE staff_id = ?", (doctor_id,))
        doc_row = cursor.fetchone()
        rating = doc_row['rating'] if doc_row and doc_row['rating'] else 5.0
        
        # 3. Pending Lab Results (Real count)
        cursor.execute("""
            SELECT COUNT(*) as count FROM lab_orders
            WHERE ordered_by = ? AND status IN ('Pending', 'Sample_Collected', 'In_Progress')
        """, (doctor_id,))
        pending_labs = cursor.fetchone()['count']
        ...
        cursor.execute("""
            ...
            JOIN lab_orders lo ON lr.lab_order_id = lo.id
            WHERE lo.ordered_by = ? AND lr.is_critical = 1
        """, (doctor_id,))
...
def get_admin_analytics():
    ...
    try:
        ...
        # Build month labels using Python to avoid SQLite strftime %% issues
