...
const navItems = [
  ...
  { name: 'Vital Signs', href: '/nurse/vitals', icon: HeartPulse },
      ...
      <aside className={`fixed left-0 top-16 h-[calc(100vh-4rem)] bg-white border-r transition-all duration-300 z-40 ${sidebarOpen ? 'w-64' : 'w-0 overflow-hidden'}`}>
        <nav className="p-4">
          <div className="mb-4 px-2">
            <p className="text-xs font-medium text-muted-foreground uppercase">Nurse</p>
      ...
      <main className={`pt-16 min-h-screen transition-all duration-300 ${sidebarOpen ? 'ml-64' : 'ml-0'}`}>
        <div className="p-6">
          <Routes>
            ...
            <Route path="medication" element={<MedicationAdmin />} />
          </Routes>
        </div>
      </main>
    </div>
