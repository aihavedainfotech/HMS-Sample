...
const PatientHome = () => {
  ...
  // Real-time: refresh appointments when backend emits appointment/queue updates
  useEffect(() => {
    const onAppointmentUpdate = () => {
      fetchDashboardData();
    };
    socket.on('appointment_approved', onAppointmentUpdate);
    socket.on('queue_status_updated', onAppointmentUpdate);
    socket.on('new_appointment', onAppointmentUpdate);
    return () => {
      socket.off('appointment_approved', onAppointmentUpdate);
      socket.off('queue_status_updated', onAppointmentUpdate);
      socket.off('new_appointment', onAppointmentUpdate);
    };
  }, []);
