"""
Migrate Missing Staff Accounts to PostgreSQL - Fixed Version
"""
...
def migrate_missing_staff():
    ...
    # Missing staff from SQLite with unique emails
    missing_staff = [
        {
            'staff_id': 'DOC002',
            'first_name': 'Susan',
    ...
    try:
        ...
        for staff in missing_staff:
            try:
                ...
                cursor.execute("SELECT email FROM staff WHERE email = %s", (staff['email'],))
                if cursor.fetchone():
                    # Generate unique email
                    staff['email'] = f"{staff['staff_id'].lower()}@hospital.com"
                    logger.info(f"📧 Email conflict, using: {staff['email']}")
