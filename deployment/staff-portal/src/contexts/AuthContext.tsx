...
interface AuthUser {
  staff_id: string;
  name: string;
  role: 'Doctor' | 'Receptionist' | 'Pharmacist' | 'Lab_Technician' | 'Nurse' | 'Admin' | 'Admission' | 'Billing' | 'Front_Office';
  department?: string;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (staffId: string, password: string, department?: string): Promise<boolean> => {
    try {
      ...
      // Extract user data from nested staff object
      const staffData = data.staff || {};
      const userData: AuthUser = {
        staff_id: staffData.staff_id || data.staff_id,
        name: `${staffData.first_name || ''} ${staffData.last_name || ''}`.trim() || data.name,
        role: staffData.role || data.role,
        department: staffData.dept_name || data.department,
        sub_department: staffData.sub_department || data.sub_department,
        email: staffData.email || data.email,
      };
